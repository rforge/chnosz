...
With the CHNOSZ package attached, use `?thermo` in R to view the documentation for the format of the data files.
...
The sections below correspond to CSV data files loaded by `data(thermo)`.
In each section, the primary references (`ref1`) are documented in chronological order.
If any secondary references are present (`ref2`), they are listed as bullet points under the primary source.
Additional symbols show whether the data were present in the earliest of sprons92.dat (ø), slop98.dat (\*), slop07.dat (†), or slop15.dat (‡).

Any additional comments are placed at the beginning of each section.
...
## Solids {.tabset .tabset-fade .tabset-pills}

### `r setfile("Helgeson_cr.csv")`
```{r Helgeson_cr, results="asis", echo=FALSE}
cat("Note: chamosite,7A, witherite, and ZnCl4-2 were present in sprons92.dat but not in slop98.dat, and are not included in CHNOSZ.")
```
