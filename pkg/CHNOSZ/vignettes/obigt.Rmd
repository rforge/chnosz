...
setfile <- function(csvfile, dat=NULL) {
  ...
  # don't count cr2, cr3, cr4 as separate minerals Helgeson_cr.csv
  if(csvfile=="Helgeson_cr.csv") dat <- dat[dat$state %in% c("cr", "cr1"), ]
...
filerefs <- function(csvfile, dat=NULL, message=FALSE) {
  ...
  xxx <- lapply(seq_along(tab), function(i){
    ...
    if(grepl("[S92]", names(tab)[i], fixed=TRUE)) note <- paste0(note, "(\\*)")
    if(grepl("[S98]", names(tab)[i], fixed=TRUE)) note <- paste0(note, "(†)")
...
This [vignette](index.html) documents the sources of thermodynamic data in CHNOSZ version `r sessionInfo()$otherPkgs$CHNOSZ$Version`.
See `?thermo` for a description of the format of the data files.

Most of the documentation here is automatically generated based on the citations in `thermo$obigt` and the notes in `thermo$refs`.
The CSV data files are documented in the order that they are added to the database by `data(thermo)`.
...
Additional symbols show if the data were present in the earliest of sprons92.dat (\*), slop98.dat (†), or slop15.dat (‡).
...
```{r Dick_aq, results="asis", echo=FALSE}
cat("Corrected values of HKF parameters *a*<sub>2</sub> and *a*<sub>4</sub> for [-CH2NH2] are used here; they were incorrectly printed as zero in Table 6 of @DLH06.")
```
...
```{r Helgeson_cr, results="asis", echo=FALSE}
cat("Note: chamosite,7A, witherite, and ZnCl4-2 were present in sprons92.dat but not in slop98.dat, and are not listed here.")
```
...
cat('"Parameters given provide smooth metastable extrapolation of one-bar steam properties predicted by the @HGK84 equation of state to temperatures < the saturation temperature (99.632 C)." -- @Joh92a')
```
