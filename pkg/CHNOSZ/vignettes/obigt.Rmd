...
setfile <- function(csvfile, dat=NULL) {
  ...
  if(csvfile=="CHNOSZ_aq.csv") file <- system.file(paste0("extdata/OBIGT/", csvfile), package="CHNOSZ")
  else file <- system.file(paste0("extdata/OBIGT/", csvfile, ".xz"), package="CHNOSZ")
...
cat("This file contains provisional data updates that are not loaded by default (using `data(thermo)`). To load these data, run `add.obigt()` with the name of one of the species below.\n\n")
...
`r length(used) - nrow(read.csv(system.file("extdata/OBIGT/CHNOSZ_aq.csv", package="CHNOSZ")))` of `r nrow(thermo$obigt)` entries in `thermo$obigt` are documented here.
