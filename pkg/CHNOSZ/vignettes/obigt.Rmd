...
cat("These files contain provisional data updates that are not loaded by default (using `data(thermo)`). To load these data, run `add.obigt('CHNOSZ_aq')` or `add.obigt('DEW_aq')`.\n\n")
```
...
cat("The Deep Earth Water (DEW) model extends the applicability of the revised HKF equations of state to 60 kbar. Accuracy of the thermodynamic calculations at these conditions is improved by revised parameters for some species, as described by [Sverjensky et al., 2014](https://doi.org/10.1016/j.gca.2013.12.019).<hr>")
```
