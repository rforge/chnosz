...
This vignette demonstrates [<span style="color:green">`EOSregress()`</span>](http://www.chnosz.net/manual/EOSregress.html) and related functions for the regression of heat capacity and volumetric data to obtain "equations of state" coefficients.
...
@HKF81 published the original description of the equations; for `r Cp0` the equation is
...
The terms in `Cplm` correspond to `r c1`, `r c2` and ω in the HKF equations.
We use <span style="color:green">`EOSplot()`</span> to plot the data and fitted line and show the coefficients in the legend.
...
Data given for `r h4sio4` by @Ste01 (`H4SiO4_Ste01`) are contained in a supplemental file:
