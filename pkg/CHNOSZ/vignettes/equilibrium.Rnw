...
\texttt{diagram(..., normalize=TRUE)} starting with version 0.9.8
of CHNOSZ.%
}.
...
function introduced in version 0.9.8. Below is its output when configured
...
Here an example where the predominant species in the equilibrium assemblage
are \textbf{\Huge not} identical to those calculated the maximum affinity
method, and by experience it is not possible for the maximum affinity
