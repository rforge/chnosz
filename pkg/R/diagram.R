...
diagram <- function(affinity,what="logact",ispecies=NULL,balance=NULL,
  ...
  if(nd==1) {
    ...
    if(is.null(xlab)) xlab <- axis.label(affinity$xname, basis=affinity$basis)
    if(what == 'logact') {
      # alpha: plot degree of formation instead of logact
      if(alpha) {
        ylog <- FALSE
      ...
      if(plot.it & !add & !is.null(legend.x)) {
        ...
        if(is.na(legend.x)) {
          for(i in 1:length(A)) {
            ...
            text(xvalues[imax],aval[[i]][imax],labels=names[i],adj=adj)
          }
  ...
  if(what=="logact") {
    ...
    if(nd==2) {
      ...
      # 20090217 added test for plot.it here
      if(plot.it) {
        if(!add) {
          if(is.null(xlab)) xlab <- axis.label(as.character(affinity$xname), basis=affinity$basis)
          if(is.null(ylab)) ylab <- axis.label(as.character(affinity$yname), basis=affinity$basis)
        ...
        if(!is.null(dotted)) plot.line(out,affinity$xlim[1:2],
          affinity$ylim[1:2],dotted,col,lwd,xrange=xrange)
...
strip <- function(affinity,ispecies=NULL,col=NULL,ns=NULL,
  ...
  for(j in 1:nstrip) {
    ...
    d <- diagram(a,residue=TRUE,plot.it=FALSE,alpha=TRUE,ispecies=ispecies[[j]])
