...
There is also a publication on CHNOSZ available \citep{Dic08}, and
\href{http://www.chnosz.net}{a website}.
...
is a modified version of the flowchart from \citet{Dic08} (boxes
-- functions; ellipses -- datasets).
...
\selectlanguage{english}%
Now we can use \texttt{diagram()} to plot the relative stabilities
...
diagram(a, legend.x="bottomleft", ylim=c(-6, -2))
@
\setkeys{Gin}{width=1.0\textwidth}
...
the compositional makeup of the proteins, per residue, in terms of
the basis species.
...
\selectlanguage{english}%
There are even more examples in longex():
...
and setting \texttt{residue=FALSE} so as to not divide the protein
formulas by lengths to compare the residue equivalents, which is the
default.
...
diagram(a, balance="CO2", residue=FALSE, lwd=2, color=NULL, cex.names=1.5)
a <- affinity(pH=c(2, 10), Eh=c(-1, 1), T=100)
diagram(a, balance="CO2", residue=FALSE, lwd=2, color=NULL, col="red", names=NULL, add=TRUE)
water.lines()
...
This figure is similar to Fig. 3 of \citet{Dic09}. Some of the differences
in positions of the lines can be attributed to updated parameters
for the methionine sidechain group \citep{LD12} that are used in
the current version of CHNOSZ.
...
diagram(a, names=locations, ylim=c(-5,-3), legend.x=NA,
  col=mycolor, lwd=2, loga.balance=0)
...
\begin{small}
\setkeys{Gin}{width=0.6\textwidth}
<<bufferplot, fig=TRUE, results=hide, width=5, height=4>>=
...
bufferline <- function(buffer, ixlab) {
  ...
  lines(a$xvals, a$H2)
  text(a$xvals[ixlab], a$H2[ixlab], buffer)
}
bufferline("FeFeO", 20)
bufferline("QFM", 38)
bufferline("PPM", 102)
...
for(logact in c(-6, -10, -15)) {
  ...
  text(a$xvals[13], mean(sapply(d$logact, c)[13, ]), logact)
}
legend("topright", legend = c(describe.property("P", 300),
  describe.basis(ibasis=c(2,4)), "minerals", "HCN", "formaldehyde"), 
  lty=c(NA, NA, NA, 1, 2, 3), bg="white")
@
\setkeys{Gin}{width=1.0\textwidth}
\end{small}
...
of species calculated by \texttt{diagram()}. In this example, the
coefficient of variation of the activities of the amino acids is plotted
as a function of $\log f_{\mathrm{O_{2}}_{\left(g\right)}}$ and $\log a_{\mathrm{CO_{2}}_{\left(aq\right)}}$.
...
d <- diagram(a, balance=1, mam=FALSE, plot.it=FALSE)
r <- revisit(d)
title(main=paste("CV minimum =", round(r$extval, 2)))
@
\setkeys{Gin}{width=1.0\textwidth}
\end{small}

\selectlanguage{english}%
The \texttt{balance=1} in \texttt{diagram()} means the relative stabilities
are calculated using the formation reactions written per mole of amino
acid (not conserving e.g. $\mathrm{CO_{2}}$ which would be the default
behaviour in this system). Note the star showing the conditions where
the coefficient of variation is minimized. Let's look at the fractional
equilibrium abundances of the amino acids at these conditions.
...
a <- affinity()
par(mfrow=c(1, 2))
d <- diagram(a, balance=1, alpha=TRUE, names=aminoacids(1, species()$name))
plot.new()
