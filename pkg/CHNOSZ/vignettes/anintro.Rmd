...
ZClab <- axis.label("ZC")
nO2lab <- expression(italic(n)[O[2]]/residue)
nH2Olab <- expression(italic(n)[H[2]*O]/residue)
lapply(c("CHNOS", "CEQ"), function(thisbasis) {
  basis(thisbasis)
  pb <- protein.basis(aa)
  nO2 <- pb[, "O2"]/pl
...
By projecting the compositions of proteins into the "CEQ" set of basis species, *n*<sub>`r o2`</sub>/residue emerges as a strong indicator of oxidation state, while *n*<sub>`r h2o`</sub>/residue is a relatively non-correlated (i.e. independent) compositional variable.
These independent variables underlie compositional and thermodynamic analyses that can distinguish the effects of oxidation and hydration state in proteomic experiments [@Dic17].

## Normalizing for different lengths

## Adding amino acid data

In the Rubisco example above, we saw the use of `read.fasta()` to read amino acid sequences from a FASTA file.
There are some other methods of adding proteins to CHNOSZ.
`read.aa()` is used to read amino acid compositions from a CSV file with the same columns that are present in `thermo$protein`:
